...
int BalanceTreeTable::DelBalance(TreeNode*& p, TKey key)
{
    ...
    {
        DataCount;
        if (p->pLeft == NULL && p->pRight == NULL) // удаление листа дерева
        {
            delete p;
            p = NULL;
            res = -1;
        }
        else if (p->pLeft != NULL && p->pRight == NULL) // удаление вершины с одним потомком слева
        {
        ...
        else if (p->pLeft == NULL && p->pRight != NULL) // удаление вершины с одним потомком справа
        {
