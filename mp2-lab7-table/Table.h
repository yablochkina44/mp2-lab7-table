...
struct Record
{
	TKey Key;
	TValue Value;
	Record(TKey k = -1, TValue Val = "") {
		Key = k;
		Value = Val;
	}

	virtual int operator==(const Record& tr) { return Key == tr.Key; }
	virtual void operator=(const Record& tr) { Key = tr.Key; }
	virtual int operator!=(const Record& tr) { return Key != tr.Key; }
	virtual int operator>(const Record& tr) { return Key < tr.Key; }
	virtual int operator<(const Record& tr) { return Key > tr.Key; }
	virtual int operator>=(const Record& tr) { return Key <= tr.Key; }
...
{
protected:
	int DataCount=0; // кол-во записей в таблице
	int Efficiency=0; // показатель эффективности выполнении€ операций
public:
	...
	//наши
	virtual bool FindRecord(TKey)  = 0;//найти запись
